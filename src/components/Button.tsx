interface ButtonProps {
  error: boolean;
  onClick: () => void;
}

export default function Button({ error, onClick }: ButtonProps) {
  return (
    <button
      className={`${error ? 'bg-red-600' : 'bg-violet-600'} flex items-center rounded-e-lg gap-2 font-medium md:gap-4 justify-center text-white absolute right-0 top-0 bottom-0 px-2 md:px-0 md:w-[268px]`}
      type="button"
      onClick={onClick}
      disabled={error}
    >
      <svg
        width="20"
        height="14"
        viewBox="0 0 20 14"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M16.944 6.112C16.507 2.67 13.56 0 10 0C7.244 0 4.85 1.61 3.757 4.149C1.609 4.792 0 6.82 0 9C0 11.657 2.089 13.815 4.708 13.971V14H15.99V13.997L16 14C18.206 14 20 12.206 20 10C19.9988 9.10352 19.6971 8.23332 19.1431 7.5285C18.5891 6.82368 17.8148 6.32494 16.944 6.112ZM6 7H9V4H11V7H14L10 12L6 7Z"
          fill="white"
        />
      </svg>
      Download
    </button>
  );
}
